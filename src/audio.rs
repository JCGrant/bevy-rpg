...
impl Plugin for GameAudioPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugin(AudioPlugin)
            ...
            .add_system_set(
                SystemSet::on_resume(GameState::Overworld).with_system(resume_bg_music_music),
            )
            ...
            .add_startup_system(start_bg_music_music);
    }
}
...
fn resume_bg_music_music(audio: Res<Audio>, audio_state: Res<AudioState>) {
    audio.stop_channel(&audio_state.combat_channel);
    audio.resume_channel(&audio_state.bg_music_channel);
}

fn start_combat_music(audio: Res<Audio>, audio_state: Res<AudioState>) {
    audio.pause_channel(&audio_state.bg_music_channel);
...
fn volume_control(
    ...
    audio.set_volume_in_channel(audio_state.volume, &audio_state.bg_music_channel);
}

fn start_bg_music_music(audio: Res<Audio>, audio_state: Res<AudioState>) {
    audio.play_looped_in_channel(
        audio_state.bg_music_handle.clone(),
        &audio_state.bg_music_channel,
    );
}

fn load_audio(mut commands: Commands, audio: Res<Audio>, assets: Res<AssetServer>) {
    let bg_music_handle = assets.load("bip-bop.ogg");
    let combat_handle = assets.load("ganxta.ogg");
    let hit_handle = assets.load("hit.wav");
    let reward_handle = assets.load("reward.wav");

    let bg_music_channel = AudioChannel::new("bg_music".to_string());
    let combat_channel = AudioChannel::new("combat".to_string());
    let sfx_channel = AudioChannel::new("sfx".to_string());
    let volume = 0.5;

    audio.set_volume_in_channel(volume, &bg_music_channel);
    ...
    commands.insert_resource(AudioState {
        bg_music_handle: bg_music_handle,
